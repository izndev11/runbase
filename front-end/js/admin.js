...
function setEditMode(evento) {
  ...
  if (descricaoEl) descricaoEl.value = evento.descricao || "";
  ...
  const categoriasTexto = Array.isArray(evento.categorias)
    ...
    : "";
  const categoriasInputEl = document.getElementById("adminCategoriasInput");
  const categoriasEl = document.getElementById("adminCategorias");
  if (categoriasInputEl) categoriasInputEl.value = categoriasTexto;
  if (categoriasEl) categoriasEl.value = categoriasTexto;
...
async function salvarEvento(event) {
  ...
  const categoriasInputEl = document.getElementById("adminCategoriasInput");
  const categoriasEl = document.getElementById("adminCategorias");
  const categoriasInput = (categoriasInputEl?.value || categoriasEl?.value || "").trim();
  const pix_chave = pixChaveEl ? pixChaveEl.value.trim() : "";
  const pix_tipo = pixTipoEl ? pixTipoEl.value : "";
  const pix_beneficiario = pixBeneficiarioEl ? pixBeneficiarioEl.value.trim() : "";
