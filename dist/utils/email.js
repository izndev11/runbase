...
exports.sendBoasVindasEmail = sendBoasVindasEmail;
exports.sendInscricaoEmail = sendInscricaoEmail;
exports.sendPagamentoEmail = sendPagamentoEmail;
...
async function sendBoasVindasEmail(params) {
    if (!canSendEmail())
        return;
    const html = `
...
async function sendInscricaoEmail(params) {
    if (!canSendEmail())
        return;
    ...
    <div style="font-family: Arial, sans-serif; color: #111;">
      ...
      ${params.opcaoTitulo
        ? `<p><strong>Opção:</strong> ${params.opcaoTitulo}${params.opcaoTipo ? ` (${params.opcaoTipo})` : ""}${params.opcaoDistanciaKm ? ` • ${params.opcaoDistanciaKm} km` : ""}</p>`
        : ""}
...
async function sendPagamentoEmail(params) {
    if (!canSendEmail())
        return;
    const valorFmt = typeof params.valor === "number"
